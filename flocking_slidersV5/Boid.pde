...
class Boid {
  ...
  void run(World world){
    render(true);
    //flock(world.flock.boids, world.sloths);
    update();
    player();
    borders();
    ArrayList<Sloth> sloths = world.sloths.sloths;
    for(int i = 0; i < sloths.size(); i++){
      if(PVector.sub(sloths.get(i).loc,this.loc).mag()<100)
      sloths.get(i).display(); 
    }
    ArrayList<Boid> boids = world.flock.boids;
    for(int i = 0; i < boids.size(); i++){
      if(PVector.sub(boids.get(i).loc,this.loc).mag()<100)
      boids.get(i).render(false); 
    }
  } 
